...
import { Calendar, Clock, MapPin, Users, ArrowRight, BookOpen, Shield, Target, Coffee, Globe, Video } from 'lucide-react';
...
export function EventCard({ event, index = 0 }: EventCardProps) {
  ...
  // Eğitmen adını yeni speaker nesnesinden veya eski instructor string alanından güvenlice alıyoruz
    ...
    >
      ...
      >
        <div className="absolute top-4 right-4 z-10 flex items-center gap-2">
          {/* Eğer geçmiş bir etkinlikse ve YouTube linki varsa kırmızı bir 'RECORDED' etiketi gösteriyoruz */}
          {!isUpcoming && event.youtubeUrl && (
            <div className="flex items-center gap-1 px-2 py-1 bg-red-500/20 border border-red-500/50 rounded-full animate-pulse">
              <Video className="w-3 h-3 text-red-400" />
              <span className="text-[10px] font-mono text-red-400 font-bold">RECORDED</span>
            </div>
          )}
        ...
        <div className="space-y-2 mb-4">
          ...
          {instructorName && (
            <div className="flex items-center gap-2 text-custom-cyan/90">
              ...
              <span className="font-mono text-sm">by {instructorName}</span>
            </div>
          )}
        </div>
