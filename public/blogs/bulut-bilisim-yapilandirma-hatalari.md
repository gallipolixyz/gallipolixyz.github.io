# Bulut Yapılandırma Hataları El Kitabı: Sıfırdan Uzmanlığa Güvenli Bulut Altyapısı

## 1. Giriş: Bulut Güvenliğindeki Büyük Yanılgı ve Ortak Sorumluluk

Bulut teknolojilerine geçiş yapan kurumsal yapıların düştüğü en büyük stratejik hata, güvenliği bir "BT problemi" olarak görüp bulut sağlayıcısının (AWS, Azure, GCP) her şeyi otomatik olarak koruyacağını varsaymaktır. Bu illüzyon, iş sürekliliğini ve kurumsal itibarı doğrudan tehdit eden bir risk zeminidir. Oysa modern bulut güvenliği, sorumluluğun keskin hatlarla ayrıldığı bir doktrin üzerine kuruludur. AWS tarafından tanımlanan Ortak Sorumluluk Modeli (Shared Responsibility Model), bu güvenliğin iki temel direği olduğunu söyler:
...
* **Bulutun İçindeki Güvenlik (Security IN the Cloud):** Veri şifreleme, ağ yapılandırması (Security Groups), kimlik yönetimi (IAM) ve müşteri verisinin korunmasından tamamen siz sorumlusunuz.

Analitik veriler, bulut ihlallerinin yaklaşık %95'inin sağlayıcı kaynaklı değil, kullanıcıların hatalı yapılandırmalarından (misconfiguration) kaynaklandığını göstermektedir. Bu veri, güvenliğin teknik bir detay değil, finansal ve reputasyonel sonuçları olan bir kurumsal risk yönetimi meselesi olduğunu sentezlemektedir.

> **Stratejik Görünürlük Notu:** Altyapı görünürlüğü, güvenliğin alfabesidir. Envanterini çıkaramadığınız, trafiğini izleyemediğiniz ve yapılandırmasını kodla denetleyemediğiniz bir kaynağı koruyamazsınız. Görünürlük eksikliği, saldırgan için bir "arka kapı" değil, "ana giriş" hükmündedir.
...
### S3 Kovaları: Kapı Altındaki Paspas
* **Hata:** S3 (Simple Storage Service) gibi depolama alanlarının "Public" bırakılması.
* **Analoji:** S3 kovanızı halka açık bırakmak, şirketinizin tüm gizli belgelerini içeren dosya dolabını sokağın ortasına koyup üzerine "Bakmak Serbest" yazmaya benzer.
...
* **PRO TIP:** Root hesabını günlük işler için asla kullanmayın. Root hesabını fiziksel bir kasada saklanan donanım tabanlı bir MFA ile mühürleyin.
...
| **3306/5432** | DB | Veri sızıntısı ve SQLi. | İnternete kapatın, sadece uygulama katmanına açın. |
...
## 3. İleri Düzey El Defteri: Kritik AWS RDS Yapılandırma Hataları
...
12. **Multi-AZ Yapılandırmasının Olmaması**
    ...
    * *Remediation:* `--multi-az` bayrağını aktif ederek yüksek kullanılabilirlik sağlayın.
...
14. **RDS'in Public Subnet'te Olması**
...
> KMS güvenliğinde **Pattern 04 (Pending Deletion Window)** hayati önemdedir. Bir anahtar silinmek üzere işaretlendiğinde (7-30 gün arası bekleme), bu süre dolduğunda geri dönüşü yoktur ve o anahtarla şifrelenmiş veri sonsuza dek kaybolur. Ayrıca **Pattern 05**, CloudTrail üzerinde KMS event takibinin yapılmamasıdır. Bir anahtar üzerinden yapılan başarısız `kms:Decrypt` denemeleri, bir saldırganın verinizi çözmeye çalıştığının en büyük kanıtıdır. 
...
Geleneksel veri merkezi mantığı olan "kalın duvarlı kale" (perimetre güvenliği) bulutta iflas etmiştir. Bulutta ağ artık yazılımdır ve savunma dinamik olmalıdır.
...
* **Bulut Yerlisi:** Egress Filtering (Çıkış Filtreleme) ve VPC Flow Logs kullanımı. Sunucuların internete değil, sadece spesifik API uç noktalarına erişmesini sağlamak.

> **PRO TIP:** Veri sızdırma (Exfiltration) riskini azaltmak için IP bazlı değil, FQDN (alan adı) bazlı filtreleme yapın. Kritik veritabanlarını internet erişimi olmayan "Isolated Subnet"lerde tutun ve servislerle iletişimi sadece VPC Endpoints (AWS PrivateLink) üzerinden sağlayın.
